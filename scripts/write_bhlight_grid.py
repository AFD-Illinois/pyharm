...
dname = sys.argv[1]
dump = load_dump(dname, calc_derived=False, add_ghosts=True)
G = dump.grid
coords = G.coords

outf = h5py.File("grid_vis.h5", "w")
...
gcon3 = G.gcon[locus, :, :, b, b, None].repeat(G.NTOT[3], axis=-1).transpose((2, 3, 4, 0, 1))
gcov3 = G.gcov[locus, :, :, b, b, None].repeat(G.NTOT[3], axis=-1).transpose((2, 3, 4, 0, 1))
gdet3 = G.gdet[locus, b, b, None].repeat(G.NTOT[3], axis=-1)
lapse3 = G.lapse[locus, b, b, None].repeat(G.NTOT[3], axis=-1)
