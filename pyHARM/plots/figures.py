...
def simplest(fig, dump, diag, plotrc, type="both", var='rho'):
    """Simplest movie: show density without color bars for outreach animations
...
def prims(fig, dump, diag, plotrc, type="poloidal"):
    ax_slc = lambda i: plt.subplot(2, 4, i)
    if type == "poloidal":
        fn = plot_xz
    else:
    ...
    for i,var in enumerate(['RHO', 'UU', 'U1', 'U2', 'U3', 'B1', 'B2', 'B3']):
        fn(ax_slc(i+1), dump, var, label="", xlabel=False, ylabel=False, xticks=[], yticks=[],
                cbar=False, **plotrc)
...
def vecs_con(fig, dump, diag, plotrc):
    ...
    for i,var in zip((1,2,3,4,5,6,7,8), ("u^0", "u^r", "u^th", "u^3","b^0", "b^r", "b^th", "b^3")):
        ...
        plot_xz(ax_slc(i), dump, np.log10(-dump[var]), label=var,
                        vmin=vmin, vmax=vmax, cmap='Blues', window=window, native=USEARRSPACE)
